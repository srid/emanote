...
Append `#heading-id` to a wiki-link to jump to a specific heading on the target page — e.g. `[[markdown#wikilink]]` renders as [[markdown#wikilink]] and lands on the Wiki Links section of [[markdown]]. The fragment matches Pandoc's auto-generated heading id (the slugified, lowercased form), the same as regular Markdown anchor links. Custom titles still work: `[[markdown#wikilink|jump to wiki-links]]` renders as [[markdown#wikilink|jump to wiki-links]].
