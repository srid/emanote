...
Vendored from pandoc-ext/diagram (https://github.com/pandoc-ext/diagram),
MIT-licensed. See `LICENSE` next to this file for the upstream notice.

Emanote bundles this filter so notes can opt in via the render-time
HTML Lua filter phase. A small Emanote extension at the bottom of this
file walks `pandoc.Image` elements whose `mediabag` entry is SVG and
replaces them with a `pandoc.RawInline` containing the literal SVG.
That keeps the rendered HTML offline-viewable without a separate
`--extract-media` step. Generated images of other MIME types are left
as `pandoc.Image` nodes for Pandoc's HTML writer to handle normally.
]]
...
-- Emanote: rewrite mediabag-backed SVG images to inline `<svg>` so the
-- rendered HTML works offline. The mediabag entry is created by
-- `code_to_figure` above (one `applyFilters` call shares the mediabag
-- across this filter's two passes), so the lookup hits whenever the
-- engine produced an SVG.
local function inline_svg (img)
  local mt, content = pandoc.mediabag.lookup(img.src)
  if mt == 'image/svg+xml' and content then
