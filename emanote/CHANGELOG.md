...
- **Inline-SVG diagrams from fenced code blocks, plus a Lua-filter error protocol** ([#625](https://github.com/srid/emanote/issues/625)): notes can opt into `lua-filters/diagram.lua` (a vendored, lightly-extended [`pandoc-ext/diagram`](https://github.com/pandoc-ext/diagram)) from their `pandoc.filters.render.html` frontmatter, and `d2`/`cetz` fenced code blocks render to inline `<svg>` at HTML render time. The wrapped Emanote binary carries `d2`, `typst`, and an offline `@preview/cetz:0.3.4` package cache so diagrams render in static sites, the live server, and air-gapped readers without a CDN or external `--extract-media` pass. Other engines the upstream filter supports (mermaid, graphviz, plantuml, tikz, asymptote) still work if the relevant binary is on `PATH` or its `_BIN` env var is set. When an engine fails — typst stderr, d2 stderr, etc. — the filter emits an Emanote-styled `emanote:error:lua-filter` Div next to the offending source instead of returning `nil`; the same red-banner styling as the top-of-page "Pandoc Lua filter error" banner, but located where the failing diagram was. That Div shape is now a documented protocol: Emanote injects an `emanote` global into every filter's Lua chunk (parse-time *and* render-time) exposing `emanote.error_block { title, message, source?, source_class? }` so user-written filters share one builder. A new `--allow-broken-lua-filters` CLI flag (Nix option `allowBrokenLuaFilters`) downgrades the static-build abort on these errors to a logged warning, so notebooks that deliberately ship error-protocol demos can build cleanly. See [[diagrams]] for the user-facing diagrams page and [[writing-filters]] for the filter-author guide.
