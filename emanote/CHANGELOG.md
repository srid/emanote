...
- Cyclic note embeds (`![[a]]` in `b.md` together with `![[b]]` in `a.md`, or a self-embed) now render an inline `↺ Cyclic embed: <title>` placeholder at the point the cycle would close, instead of expanding the embed without a fixpoint and either hanging the live preview or producing arbitrarily deep nested output. The custom Pandoc renderers thread a `Set LMLRoute` ancestor stack through `mkRenderCtxWithPandocRenderers` and `embedResourceRoute`; on a hit the renderer falls back to a `.emanote\:error\:cyclic-embed` div (closes [#362](https://github.com/srid/emanote/issues/362)).
