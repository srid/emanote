...
- Inline broken-link rendering moved from a hard-coded Pandoc AST to a new `templates/components/broken-link.tpl` Heist template. Notebook authors can drop in their own copy to fully replace the strikethrough/icon/aside structure, or just override the `emanote:broken-link`, `emanote:broken-link__text`, `emanote:broken-link__icon`, and `emanote:broken-link__aside` class hooks via CSS. Tooltip and aside link text are looked up via the new `brokenLink` / `brokenLinkBacklinks` / `brokenLinkBacklinksTooltip` `template.i18n` keys (English + French shipped). The broken-link landing page (`templates/error.tpl`) was already overridable; this completes the issue's request that broken-link rendering be user-controllable (closes [#221](https://github.com/srid/emanote/issues/221)).
