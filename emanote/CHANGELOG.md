...
- A malformed `*.yaml` file (e.g. a non-string mapping key like `[]: foo`) no longer takes the live server down with `BadInput "NonStringKey []"`; the parse error is logged and the rest of the site keeps rendering with the previous good metadata for that route. The browser-visible "global errors page" surfacing yaml errors is the proper follow-up tracked by [#285](https://github.com/srid/emanote/issues/285) — this PR addresses only the crash.
