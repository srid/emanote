...
- A malformed `*.yaml` file (e.g. a non-string mapping key like `[]: foo`) no longer takes the live server down with `BadInput "NonStringKey []"`. Parse errors are captured in the model and surfaced as a banner block on every rendered note until the file is fixed, so a bad yaml file can't fail silently (closes [#285](https://github.com/srid/emanote/issues/285)).
