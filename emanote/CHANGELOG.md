...
- A Markdown link whose label contains a URL or email address now renders as a single hyperlink instead of being split into the parent link plus separate autolinks for the embedded URLs. `commonmark-hs`'s autolink extension produces nested `Link` nodes for these labels; the parser now flattens the inner `Link` so the displayed label survives but its target is dropped (closes [#349](https://github.com/srid/emanote/issues/349); fixed upstream in [srid/commonmark-simple#2](https://github.com/srid/commonmark-simple/pull/2)).
