...
- A malformed `*.yaml` file (e.g. a non-string mapping key like `[]: foo`) no longer takes the live server down with `BadInput "NonStringKey []"`; the parse error is logged and the rest of the site keeps rendering with the previous good metadata for that route (closes [#285](https://github.com/srid/emanote/issues/285)).
