...
- **`.emanoteignore` hot-reload** (closes [#739](https://github.com/srid/emanote/issues/739)): edits to a layer's `.emanoteignore` during `emanote run` now take effect without a restart — added patterns evict matching notes from the model and the sidebar, removed patterns bring previously-hidden notes back. See [[emanoteignore]].
