...
- Pandoc Lua filters now hot-reload: editing a `.lua` file referenced from a note's `pandoc.filters` frontmatter re-parses every dependent note in place, so the rendered HTML reflects the new filter without the previous `touch foo.md` workaround. `.lua` files are tracked as their own file type (claimed before the static-file catch-all by `unionmount`'s first-match-wins) so they no longer leak into `_site/` either. Builds on the MVP from [#278](https://github.com/srid/emanote/pull/278); the cascade-from-`index.yaml` half of [#263](https://github.com/srid/emanote/issues/263) remains open.
