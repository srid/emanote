...
- **Lua filter `warn(...)` surfaces as an in-page banner**: render-time Lua filters that emit pandoc `ScriptingWarning` messages (e.g. via `warn(...)`) now route through the same `Pandoc Lua filter error` banner used for catastrophic filter failures, instead of going only to stderr. The bundled diagram filter relies on this to make engine errors (typst parse failures, `mmdc` not found, etc.) visible to the note author.
