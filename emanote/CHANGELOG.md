...
- Inline hashtag parsing now leaves all-numeric GitHub-style issue references such as `#221` as plain text, so they no longer link to tag pages or pollute the global tag index. Tags that contain digits, such as `#issue-221` or `#tag221`, still work normally.
