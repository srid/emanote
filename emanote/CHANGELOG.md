...
- **Inline-SVG diagrams from fenced code blocks** ([#625](https://github.com/srid/emanote/issues/625)): notes can opt into `lua-filters/diagram.lua` (a vendored, lightly-extended [`pandoc-ext/diagram`](https://github.com/pandoc-ext/diagram)) from their `pandoc.filters.render.html` frontmatter, and `d2`/`cetz` fenced code blocks render to inline `<svg>` at HTML render time. The wrapped Emanote binary carries `d2`, `typst`, and an offline `@preview/cetz:0.3.4` package cache so diagrams render in static sites, the live server, and air-gapped readers without a CDN or external `--extract-media` pass. Other engines the upstream filter supports (mermaid, graphviz, plantuml, tikz, asymptote) still work if the relevant binary is on `PATH` or its `_BIN` env var is set. See [[diagrams]] for the opt-in snippet and demos.
- **Diagram engine errors render in-place** ([#625](https://github.com/srid/emanote/issues/625)): when `typst`, `d2`, or another bundled engine fails on a fenced block, the bundled `lua-filters/diagram.lua` filter emits an Emanote-styled `emanote:error:lua-filter` Div next to the offending source — same red-banner styling as the top-of-page "Pandoc Lua filter error" banner, but located where the failing diagram was, so the author sees engine stderr right next to the code that produced it.
- **Lua filter hot-reload, bundled filters, and render-time filters** (closes the MVP→final half of [#263](https://github.com/srid/emanote/issues/263), tracked in [#721](https://github.com/srid/emanote/issues/721)): editing a `.lua` file referenced from any note's Markdown `pandoc.filters.parse` / `pandoc.filters.render.html` frontmatter or Org `#+PANDOC_FILTERS` keywords now refreshes every dependent note in place — no `touch foo.md` workaround required. Parse-time filters run with `FORMAT == "markdown"` and with IO-capable Lua/Pandoc APIs disabled; HTML render-time filters run with `FORMAT == "html"` and receive the note's effective metadata in `doc.meta`. The reverse-dependency index keys edges by the filter path *as written* (typically layer-relative), so a filter referenced before it exists on disk also gets an edge: creating the file later triggers re-parse. `.lua` is also now claimed as its own file type so edits route through the hot-reload path while filter sources remain wikilinkable as source files. Emanote now bundles the maintained [`pandoc-ext/list-table`](https://github.com/pandoc-ext/list-table) filter plus an Emanote-specific `wordcount.lua` demo filter, so notes can declare `pandoc.filters.parse: [lua-filters/list-table.lua]` without copying the filter into the notebook. The docs include a render-time custom `slides.lua` powering [`/lua-filters/slides`](https://emanote.srid.ca/lua-filters/slides), itself a Markdown deck *about* Lua filters. Built on `unionmount`'s new `unionMountStreaming` so the patch handler reads the running model directly without maintaining a parallel mirror.
...
- Source-code embeds (`![[foo.hs]]`, etc.) are now syntax-highlighted at build time through the same skylighting pipeline used for fenced code blocks. Previously the embed-code template emitted raw text inside `<pre><code class="haskell">` and relied on the client-side highlight.js pass that #624 had since removed, leaving every code embed unstyled. The `embed-code.tpl` template now exposes `<ema:code:block />` (a block splice that produces the full highlighted `<div><pre><code>…</code></pre>` from skylighting) instead of the old `<ema:code:content />` text splice — third-party themes that override `embed-code.tpl` need to swap the splice name and drop their own `<pre><code>` wrapper. Recognised extensions are now whatever [skylighting](https://github.com/jgm/skylighting)'s own `syntaxesByExtension` matches against its bundled Kate XML syntaxes (hundreds of languages from Ada through Zsh), instead of a curated allow-list — closes the last unchecked item of [#24](https://github.com/srid/emanote/issues/24) ("Everything else? As code block").
...
- Regular Markdown links to notes no longer require the `.md` or `.org` suffix. Extensionless targets such as `[neuron](guide/neuron)` and folder-note targets such as `[guide](guide)` now resolve to internal note routes before falling back to static files (closes [#347](https://github.com/srid/emanote/issues/347)).
...
- Inline diagnostics for broken and ambiguous wikilinks moved from hard-coded Pandoc AST in `Renderer.Url` into two new Heist templates — `templates/components/broken-link.tpl` and `templates/components/ambiguous-link.tpl` — so notebook authors can drop in their own copies to fully replace the structure, or just override the `emanote:broken-link[__text|__icon]` and `emanote:ambiguous-link[__text|__icon|__candidate]` class hooks via CSS. The "backlinks" superscript that previously appeared next to broken links in live preview is gone — clicking the broken link still goes to the missing-link landing page (`templates/error.tpl`), which already lists the backlinks. Ambiguous-link candidates render through a new `<ema:ambiguous-link:candidates><each-candidate>…</each-candidate></ema:ambiguous-link:candidates>` list-splice protocol with per-candidate `<ema:candidate:label />`, `${ema:candidate:url}`, `${ema:candidate:route}` (template composes its own tooltip), and a boolean `<ema:candidate:newtab>… <else /> …</ema:candidate:newtab>` splice (fires the first branch for non-Ema-internal candidates) so the template owns the `target="_blank"` decision. Two new `template.i18n` keys (`brokenLinkTooltip`, `ambiguousLinkTooltip`) drive the tooltip text, with English and French shipping (closes [#221](https://github.com/srid/emanote/issues/221), closes [#712](https://github.com/srid/emanote/issues/712)).
