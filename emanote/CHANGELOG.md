...
- Source-code embeds (`![[foo.hs]]`, etc.) are now syntax-highlighted at build time through the same skylighting pipeline used for fenced code blocks. Previously the embed-code template emitted raw text inside `<pre><code class="haskell">` and relied on the client-side highlight.js pass that #624 had since removed, leaving every code embed unstyled. The `embed-code.tpl` template now exposes `<ema:code:block />` (a block splice that produces the full highlighted `<div><pre><code>…</code></pre>` from skylighting) instead of the old `<ema:code:content />` text splice — third-party themes that override `embed-code.tpl` need to swap the splice name and drop their own `<pre><code>` wrapper. The list of recognised extensions also grows from ~16 entries to cover most common config and markup formats (`.json` `.yaml` `.toml` `.xml` `.html` `.css` `.scss` `.sql` `.dockerfile` `.makefile` `.diff` `.patch` `.tex` `.lua` `.scala` `.elm` `.elixir` `.erlang` `.dart` `.zig` `.fsharp` `.ocaml` …) — closes the last unchecked item of [#24](https://github.com/srid/emanote/issues/24) ("Everything else? As code block").
