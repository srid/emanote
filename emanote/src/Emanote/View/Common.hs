...
commonSplices withCtx model meta routeTitle = do
  ...
  "ema:metadata" ##
    HJ.bindJson meta
  -- Iterate the page's tags with a per-tag URL bound to the index
  -- through the same Ema URL builder Emanote uses elsewhere — so
  -- characters reserved by RFC 3986 (notably `#`, used in Zettelkasten
  -- "structure note" tags like `##§1`) are percent-encoded. See #199.
  "ema:tagsList" ## do
