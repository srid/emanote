...
import Control.Monad.Logger (MonadLoggerIO, logDebugNS, logInfoNS, logWarnNS)
...
readOrBuildStorkIndex (IndexVar indexVar) config = do
  readTVarIO indexVar >>= \case
    Just index -> do
      logDebugNS "emanote" "STORK: Returning cached search index"
      pure index
    Nothing -> do
      -- TODO: What if there are concurrent reads? We probably need a lock.
      -- And we want to encapsulate this whole thing.
      logWarnNS "emanote" "STORK: Generating search index (this may be expensive)"
      (diff, !index) <- timeIt $ runStork config
      logInfoNS "emanote" $ toText $ "STORK: Done generating search index in " <> showGFloat (Just 2) diff "" <> " seconds"
