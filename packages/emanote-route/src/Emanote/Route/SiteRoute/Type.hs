...
data VirtualRoute
  = VirtualRoute_Index
  | VirtualRoute_TagIndex [TT.TagNode]
...
decodeTagIndexR :: FilePath -> Maybe [TT.TagNode]
decodeTagIndexR fp = do
  "-" :| "tags" : tagPath <- pure $ R.unRoute $ R.decodeHtmlRoute fp
  pure $ fmap (TT.TagNode . Slug.unSlug) tagPath
...
encodeTagIndexR tagNodes =
  R.R $ "-" :| "tags" : fmap (fromString . toString . TT.unTagNode) tagNodes
